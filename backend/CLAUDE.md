...
새로운 API를 만들 때 아래 형식으로 명세를 작성한다.
...
| 상황 | 에러 코드 | HTTP 상태 |
|---|---|---|
| 이름 빈칸 | COMMON-101 | 400 |
| 이미 등록된 환자 | PATIENT-302 | 409 |
...
```jsx
// 환경별 BASE_URL은 .env 파일에서 관리
const BASE_URL = import.meta.env.VITE_API_URL;
// Local:  http://localhost:8080/api/v1
// Dev:    https://j14e205.p.ssafy.io/dev/api/v1
// Prod:   https://j14e205.p.ssafy.io/api/v1

// 환자 생성
const response = await fetch(`${BASE_URL}/patients`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: '홍길동', birthDate: '2000-01-01' })
});
...
if (result.code === 'SUCCESS' || result.code === 'CREATED') {
    ...
    // 에러: result.code 로 분기 처리
    if (result.code === 'AUTH-201') {
        // 토큰 만료 → refresh 토큰으로 재발급
    }
    alert(result.message);  // 사용자에게 에러 메시지 표시
}
```
...
- [ ] URL이 `/api/v1/`로 시작하는가?
- [ ] 리소스명이 복수형 명사 + 케밥케이스인가?
- [ ] HTTP Method가 CRUD에 맞게 사용됐는가?
- [ ] 성공 응답을 `ApiResponse.ok()` / `ApiResponse.created()`로 감쌌는가?
- [ ] 에러를 `throw new BusinessException(ErrorCode.XXX)`로 던지는가?
- [ ] 새 에러 코드는 `ErrorCode.java`에 추가했는가?
- [ ] Request DTO에 `@Valid` + 검증 어노테이션 붙였는가?
- [ ] 응답 DTO 네이밍이 컨벤션을 따르는가?
...
```bash
make local-up          # 백엔드 + DB + Redis + RabbitMQ 한 번에 시작
make local-down        # 종료
...
```bash
make data-dev-up       # 1단계: DB/Redis/RabbitMQ 시작
make dev-app-up        # 2단계: 백엔드 + 프론트 배포
make dev-app-down      # 종료 (DB는 유지됨)
```

### 프로덕션 서버 (prod)

```bash
make data-prod-up      # 1단계: DB/Redis/RabbitMQ 시작 + prod-net 생성
make infra-up          # 2단계: Nginx 프록시 시작
make prod-app-up       # 3단계: 백엔드(Blue/Green) + 프론트 배포
make prod-app-down     # 앱만 종료 (DB는 유지됨)
```

### 상태 확인 / 로그

```bash
make status            # 전체 서비스 상태 확인
make logs-nginx        # Nginx 로그
make logs-was-blue     # 백엔드 Blue 로그
make logs-was-green    # 백엔드 Green 로그
make logs-was-dev      # 개발 백엔드 로그
```

### Blue/Green 트래픽 전환

```bash
make switch-blue       # Blue 서버로 트래픽 전환
make switch-green      # Green 서버로 트래픽 전환
```

### 정리

```bash
make clean             # 안 쓰는 Docker 리소스 정리
make help              # 명령어 도움말 보기
```
